...
make_plot <- function(gene) {
  ...
  g1 <- ggplot2::ggplot(seqs.df1,
    ...
    ggplot2::scale_x_continuous(limits = c(1940L, 2025L)) +
  ...
  g2 <- ggplot2::ggplot(seqs.df2,
                        ggplot2::aes(x = codon,
                                     ...
                                     alpha = AA_val)) +
    ggplot2::geom_tile() +
    ggh4x::facet_wrap2(~AA, ncol = 1, strip.position = "right") +
    ggplot2::theme_minimal() +
  ...
  gp <- ggpubr::ggarrange(g1, g2, widths = c(3, 1), align = "h",
                          label.x = c(0, -0.1),
                          label.y = c(.99, .99),
                          labels = if(gene == "Pita1") {c("B", "C")} else {c("E", "F")})

  return(gp)
}

plot1 <- make_plot("Pita1")
plot2 <- make_plot("Piks")

# try to read in the two PNGs
img1 <- png::readPNG(here::here("figures", "haplotype_network_Pita1.png")) |>
  grid::rasterGrob(interpolate = TRUE)

img2 <- png::readPNG(here::here("figures", "haplotype_network_Piks.png")) |>
  grid::rasterGrob(interpolate = TRUE)

gpl <- ggpubr::ggarrange(ggpubr::annotate_figure(img1,
                                                 left = ggpubr::text_grob("AVR-Pita1",
                                                                          face = "italic",
                                                                          size = 15,
                                                                          rot = 90,
                                                                          family = "Arial")),
                         ggpubr::annotate_figure(img2,
                                                 left = ggpubr::text_grob("AVR-Piks",
                                                                          face = "italic",
                                                                          size = 15,
                                                                          rot = 90,
                                                                          family = "Arial")),
                         labels = c("A", "D"), heights = c(3, 2),
                         label.y = c(.99, .99), ncol = 1)
gpr <- ggpubr::ggarrange(plot1, plot2, heights = c(3, 2), ncol = 1)

gp  <- ggpubr::ggarrange(gpl, gpr, nrow = 1)

gp
ggplot2::ggsave(gp,
                filename = here::here("figures", paste0("Figure_5.png")),
                width = 7.5,
                height = 5,
                dpi = 600,
                scale = 2,
